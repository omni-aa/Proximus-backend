...
app.post("/signup", (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE username = ?", [safeUser], async (err, row) => {
...
app.post("/signin", (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE username = ?", [safeUser], async (err, user: UserRow | undefined) => {
        if (err) return res.status(500).json({ error: "Database error" });
...
io.use((socket, next) => {
    ...
    // @ts-ignore
    jwt.verify(token, JWT_SECRET, (err: any, decoded: { username: string; }) => {
        if (err) return next(new Error("Authentication error"));
        socket.data.username = (decoded as { username: string }).username;
        next();
    });
});
...
io.on("connection", (socket) => {
    ...
    socket.on("send_message", (msg) => {
        ...
        if (!safeChannel || !safeUser || (!safeMessage && !msg.fileUrl)) {
            console.warn("Blocked message: invalid data");
            return;
        }
